...
INTEGRATION NOTE:
    The router now coordinates with ai_engine for per-client
    lifecycle management. When a client disconnects, both the
    router's rate-limit state AND the ai_engine's per-client
    MediaPipe resources are cleaned up.
============================================================
"""
...
class FrameRouter:
    ...
    Features:
        ...
        - Coordinated AI engine cleanup on disconnect
    """
    ...
    def __init__(self):
        ...
        # Per-client frame counter for logging cadence
        self._frames_processed: Dict[str, int] = {}
    ...
    async def _route_to_engine(
        ...
        try:
            ...
            # Send to AI engine (real MediaPipe or placeholder)
            ...
            # Periodic processing log (every 100 frames to avoid spam)
            self._frames_processed[client_id] = (
                self._frames_processed.get(client_id, 0) + 1
            )
            frame_count = self._frames_processed[client_id]
            if frame_count % 100 == 0:
                logger.info(
                    "frame_processing_milestone",
                    client_id=client_id,
                    total_processed=frame_count,
                    total_dropped=self._frames_dropped.get(client_id, 0),
    ...
    def cleanup_client(self, client_id: str) -> None:
        ...
        Cleans up both the router's tracking state AND the
        AI engine's per-client MediaPipe resources.
        ...
        frames_processed = self._frames_processed.pop(client_id, 0)
        ...
        # Clean up AI engine state (PoseEngine, filters, etc.)
